...
type AppContainerized struct {
	App              *application.App
	containerPool    *dockertest.Pool
	postgresResource *dockertest.Resource
	natsResource     *dockertest.Resource
	fgaResource      *dockertest.Resource
}

func (t *AppContainerized) Close() {
	if err := t.containerPool.Purge(t.postgresResource); err != nil {
		log.Fatalf("could not purge postgres resource: %s", err)
	}

	if err := t.containerPool.Purge(t.natsResource); err != nil {
		log.Fatalf("could not purge nats resource: %s", err)
	}

	if err := t.containerPool.Purge(t.fgaResource); err != nil {
