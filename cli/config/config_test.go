...
func TestConfigurator_Defaults(t *testing.T) {
	...
	assert.Empty(t, config.Clouds.AWSTenants)
	assert.Empty(t, config.Clouds.AzureTenants)
	assert.Empty(t, config.Clouds.GCPTenants)

	assert.False(t, config.Okta.Enabled)
}
